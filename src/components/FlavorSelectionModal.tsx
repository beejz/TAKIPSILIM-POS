...
import { MenuItem, MenuItemFlavor } from "../types/cafe";
...
interface FlavorSelectionModalProps {
  ...
  item: MenuItem | null;
  onClose: () => void;
  onConfirm: (flavor: MenuItemFlavor) => void;
}
...
export default function FlavorSelectionModal({
  ...
  const [selectedFlavor, setSelectedFlavor] = useState<MenuItemFlavor | null>(null);
                ...
                <View className="flex-row items-center flex-1">
                  ...
                  <Text
                    className={`text-base font-semibold ${
                      selectedFlavor?.name === flavor.name ? "text-cafe-orange" : "text-gray-900"
                    }`}
                  >
                    {flavor.name}
                  </Text>
          ...
          <View className="flex-row gap-3">
            ...
            <Pressable
              ...
              className={`flex-1 py-3 rounded-lg items-center active:opacity-90 ${
                selectedFlavor ? "bg-cafe-orange" : "bg-gray-300"
              }`}
              style={{
                shadowColor: selectedFlavor ? "#FF7A00" : "transparent",
